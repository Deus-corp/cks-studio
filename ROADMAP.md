...
# Current Status (v0.5.1 — August 2026)
...
type filtering and quick-jump search, inference and fork inspection,
pipeline and agent monitoring with live control, a public gallery, an
AI chat panel wired to the same MCP tools an LLM would use, and a
dark/light theme built on a single token system. 60+ tests, TypeScript
...
- [ ] **Code-splitting** — the current single JS bundle is ~550 kB
  minified; split the AI Chat, Gallery, and export (`html-to-image`)
  code paths into async chunks.
