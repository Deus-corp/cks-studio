...
/**
 * Cmd/Ctrl+K quick-jump: fuzzy-ish search over node label/id so you can
 * reach a node by typing instead of scrolling/zooming a large canvas.
 * Selecting a result centers the viewport on it and opens the node in
 * the side panel, same as clicking it directly.
 *
 * Must be rendered as a child of <ReactFlow> (or under a
 * ReactFlowProvider) so useReactFlow() has a context to read.
 */
export function GraphSearchPalette({
  ...
  /** Optional controlled open state, so a trigger button elsewhere can open it. */
  isOpen?: boolean
  ...
  return (
    ...
    >
      {/* Click-outside-to-close backdrop; the actual dialog content below stops propagation. */}
      <button
        type="button"
